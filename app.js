...
function get_limit_size() {
  if (conf.max_size === "") return 1024 * 1024 * 10;
  if (typeof conf.max_size !== "string") conf.max_size = conf.max_size + "";
  ...
  if (conf.max_size.slice(-1) === "K") return parseInt(conf.max_size) * 1024;
...
function delete_old(file) {
  ...
  fs.readdir(dirName, function (err, files) {
    ...
    for (i = 0, len = files.length; i < len; i++) {
      if (files[i].indexOf(fileBaseName) >= 0) rotated_files.push(files[i]);
    }
...
function proceed(file) {
  ...
  // pipe all stream
  if (COMPRESSION) readStream.pipe(GZIP).pipe(writeStream);
  ...
  writeStream.on("finish", function () {
    ...
    fs.truncate(file, function (err) {
      ...
      if (typeof RETAIN === "number") delete_old(file);
    });
  });
}
...
function proceed_file(file, force) {
  ...
  fs.stat(file, function (err, data) {
    ...
    if (data.size > 0 && (data.size >= SIZE_LIMIT || force)) proceed(file);
  });
}
  ...
  setInterval(function () {
    ...
    pm2.list(function (err, apps) {
      ...
      apps.forEach(function (app) {
        ...
        if (
          typeof app.pm2_env.axm_options.isModule !== "undefined" &&
          !ROTATE_MODULE
        )
          return;
